...
public:
    ...
    int pack(Local<Value> value, const int nestLimit = DEFAULT_RECURSE_LIMIT) {
        ...
        if (value->IsInt32() || value->IsUint32()) {
            int number = value->Int32Value(Nan::GetCurrentContext()).ToChecked();
            if (number >= 0 && number <= 255) {
                ...
                auto uNum = (unsigned long long)value->Uint32Value(Nan::GetCurrentContext()).ToChecked();
                ret = erlpack_append_unsigned_long_long(&pk, uNum);
            }
        }
        else if(value->IsNumber()) {
            double decimal = value->NumberValue(Nan::GetCurrentContext()).ToChecked();
